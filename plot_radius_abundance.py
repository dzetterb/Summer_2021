def plot_radius_abundance(filename = 'None', nuc_names_wanted = "None", figurename = "None", listoflabels = "None", nuc_names_add = 'None', endab_file = 'None', xaxis = "mass_coordinate", xmin_max = [0, 10], ymin_max = [10e-8, 10e-3], yscalelog = True):
        """
        ...
        plt.xlabel("Mass Coordinate (M$\odot$)")
        
        ax1.set_position([box.x0, box.y0, box.width * 0.995, box.height])
        plt.title("$^{10}$Be Mass Fraction vs Mass Coordinate")
        
        
        #Create Legend
        if listoflabels == 'None':
            label_list = []
            for file in filename:
